...
- **No `tailwind.config.ts`** (Bölüm XI-B.3 expects one). Tailwind v4 is
  CSS-first — theme tokens live in `src/styles/globals.css` via `@theme`,
  which is also the path `components.json` points shadcn at.
...
The single 9,000-line spec has been split into `spec/` (18 files), a deviation
log, and a repos-crossing `handoff/` channel, because one file was serving
three reading patterns and costing roughly 65k tokens a session.

The bundle is **untracked and undistributed**. It carries its own instructions
(`README-BUNDLE.md`, `_MIGRATION.md`, `_PLACEMENT.md`) and a migration prompt
at `_prompts/frontend-migration-prompt.txt` that the user pastes when ready.
`_prompts/`, `_MIGRATION.md` and `_PLACEMENT.md` are explicitly **not** meant
to be committed.

Do not distribute it on your own initiative — it rewrites where every document
...
**`AtomEditor` is built**, with `ImportanceSlider`, `LockToggles` and
`RichText` under it, and the `ui` primitives they need (`label`, `textarea`,
`slider`, `switch`) written against the installed `radix-ui` package rather
than fetched by the shadcn CLI.

- **Two autosaves per atom, not one.** Controls go through
  `PATCH /atoms/{id}`, wording through the variant endpoint, and the two
  version independently (D.9 · 17) — so a slider dragged while a sentence is
  still settling must not cancel it.
- **⚠️ Text editing is plain text and it drops marks.** The mark-aware editor
  is the lazily-loaded component rule 4 names and has not been built. What
  makes the gap acceptable is that it is _stated_: a warning appears when the
  atom actually has marks, before the save, never after (P8). Do not remove
  that warning without building the editor that makes it untrue.
- **Aria props on a Radix `Slider` belong on the thumb.** Radix puts
  `role="slider"` there, so `aria-labelledby` left on the root labels a bare
  `div` and the control has no name; `aria-valuetext` is not even allowed
  where there is no role. `ui/slider.tsx` forwards all three. Caught by
  `jest-axe`, not by review — which is the argument for keeping an axe
  assertion on every component that wraps a primitive.
- **jsdom has no `ResizeObserver`**, and Radix reaches for one in anything
  that measures itself. `tests/setup.ts` stubs it. A real polyfill would be no
