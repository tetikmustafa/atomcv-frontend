# İnşa Notları — Aşama 1 (frontend, kapandı)

> Aşama 1'in tam inşa kaydı. **Rutin okunmaz** — `rg -n "<konu>" docs/notes/archive/`.
>
> Sınırı 200'dü ve aşama boyunca beş kez büyütüldü (200 → 320 → 400 → 440 →
> 480 → 560): bölmek yerine büyütmek bilinçli bir karardı, ama asıl çözüm
> buydu — aşama kapanınca arşivlenmesi. Bir sonraki aşama küçük bir dosyayla
> ve küçük bir sınırla başlıyor.
...
### B-035 / B-036, ve Aşama 1'in son iki boşluğu

**B-035 — `sourceLanguage` gövdede zorunlu.** F-004'e "temizlensin" cevabını
verdik, backend uygulayamadı: kolon `NOT NULL DEFAULT 'en'`, yani temizlemek
Türkçe yazılmış bir profili herhangi bir baş düzenlemesinde sessizce
İngilizceye çevirirdi. Alan zorunlu oldu; başın artık hiçbir alanı merge
edilmiyor.

Bizde kırılacak yer vardı: `toUpdate` alanı **koşullu** gönderiyordu
(`profile.sourceLanguage ? {…} : {}`), yani alanı olmayan bir profilde onu
düşürür ve 400 alırdı. Koşul kaldırıldı.

**B-036 — `params.fields` artık isteğin gönderdiğini adlandırıyor**, ve
burada **ölçüm hatam çıktı.** Sözcükleme silmede iki ayrı kural var:
...
`variantId`'yi *son sözcükleme* vakasında ölçmüştüm; ikinci vakada sondam
yalnız `status`'ü basıyordu, `params`'ı hiç görmedim ve tek ölçülmüş vakadan
ikisine genelledim. Handoff'a "tek kural" diye yazdım. Backend "muhtemelen
mock'unuzdan ölçüldü" dedi — orası doğru değil, gerçek uca vurulmuştu; hata
genellemede. **Ders sonda yazarken: her dalın `params`'ını bas, yoksa
ölçmediğin dalı ölçmüş sanırsın.**

**Entry `PATCH` kuralı bir tasarım kısıtı getiriyor.** Sunucu aralığı
**yamanın sonucuna** göre denetliyor ve `params.fields` isteğin gönderdiği
uçları adlandırıyor. Yalnız değişen alanı göndermek, kullanıcının ekranda
görmediği bir alanı adlandıran bir hata üretebilirdi. Bu yüzden
`toEntryPatch` **formun gösterdiği her alanı** gönderiyor, boşları `null`
olarak — `PATCH`'te yokluk "dokunma" demek, yani boşaltmak omit ile
yapılamaz.

**Düzenleme yüzeyi (bölüm ve entry).** İnşa kılavuzunun listesinde yoktu ama
silme cascade'li: başlığı yanlış yazılmış bir bölüm için "sil ve yeniden
yarat" altındaki her şeyi götürüyor. Formlar ayrı yazılmadı — `SectionForm`
ve `EntryForm` düzenleme kipi aldı; alanlar, şema, hata kablolaması ve
etiketler birebir aynı, ikinci kopya aynı formun kendi hatalarıyla olurdu.
`kind` de düzenlenebilir, çünkü altına eklenen maddenin ne sayılacağına o
karar veriyor.

**F-003 kapandı, geçici çözüm kalktı.** Kural "`completeness` taşıyan bir
yanıt güncel bir değer taşır" — kolonun her yazmadan sonra güncel olduğu
değil. Bölüm/entry/atom uçları başı döndürmüyor, o yüzden silme hook'ları
başı invalidate etmeye **devam ediyor**; kalkan yalnız `useReplaceProfile`'ın
fazladan okuması.

**F-007 kapandı ve iş çıkardı.** Kota günü **UTC** dönüyor, `resetsAt` telde
her zaman offset'li mutlak bir an. Kota mesajları artık saati söylüyor:
UTC gece yarısı okuyucunun yerelinde `03:00` çıkıyor. `formatErrorParams`
`resetsAt`'i `Date`'e çeviriyor — **isimle**, şekle bakarak değil: `params`
telde tipsiz ve tarihe benzeyen bir dizge tarih olmayabilir.

Beklediğim başarısızlık kipi yanlıştı: ham ISO dizgesini ICU'nun `time`
argümanına vermek dizgeyi cümlede bırakmıyor, **çıplak anahtara** düşüyor —
dizi vakasının aynısı. Yani atlanırsa kullanıcı `errors.QUOTA_EXCEEDED`
okuyor. Test bunu sabitliyor.

**Playwright/Testing Library farkı ikinci kez ısırdı**, bu sefer ters yönden:
`exact: true`'yu bir birim teste taşıdım, `ByRoleOptions`'ta öyle bir alan
yok ve typecheck yakaladı. Playwright'ta gerekli, Testing Library'de hem
gereksiz hem hata.
